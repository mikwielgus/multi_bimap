...
where
    ...
    <L2R as Container>::Value:
        Remove<<R2L as Container>::Key, Output = bool> + Default + PartialEq,
    <R2L as Container>::Value:
        Remove<<L2R as Container>::Key, Output = bool> + Default + PartialEq,
    <L2R as Container>::Key: Clone,
    <R2L as Container>::Key: Clone,
{
...
mod tests {
    ...
    fn insert_allows_many_on_both_sides() {
        ...
        assert_eq!(m.left_to_right().get(&"a").map(sorted_values), Some(vec![1, 2]));
        assert_eq!(m.left_to_right().get(&"b").map(sorted_values), Some(vec![1]));
        assert_eq!(
            m.right_to_left().get(&1).map(sorted_values),
            Some(vec!["a", "b"]),
        );
        assert_eq!(m.right_to_left().get(&2).map(sorted_values), Some(vec!["a"]));
    }
    ...
    fn remove_pair_and_drop_empty_keys() {
        ...
        assert_eq!(m.left_to_right().get(&"a").map(sorted_values), Some(vec![2]));
        assert_eq!(m.right_to_left().get(&1).map(sorted_values), Some(vec!["b"]));
